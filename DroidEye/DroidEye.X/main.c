...
//  1.00aF  FG, 05 Oct 16, added logic to see if incoming data is for left or right eye
//
//***********************************************************************************
#include <pic.h>
#include <string.h>
...
/* Consider the following initialization sequence of commands to show eyes blinking and alive
00 c0 00 00 0c 00 00
00 c0 00 0c 0c 00 00
00 c0 1e 1e 1e 1e 00
00 de 21 2d 2d 21 1e -- look centre
 */
...
void columnSwitch(int colInd, int rowInd) {
    switch (colInd) {
    case 0:
        if ((visibleSymbol[rowInd] & 0x20) > 0) COL1 = 1;
        else COL1 = 0;
        break;
    case 1:
        if ((visibleSymbol[rowInd] & 0x10) > 0) COL2 = 1;
        else COL2 = 0;
        break;
    case 2:
        if ((visibleSymbol[rowInd] & 0x08) > 0) COL3 = 1;
        else COL3 = 0;
        break;
    case 3:
        if ((visibleSymbol[rowInd] & 0x04) > 0) COL4 = 1;
        else COL4 = 0;
        break;
    case 4:
        if ((visibleSymbol[rowInd] & 0x02) > 0) COL5 = 1;
        else COL5 = 0;
        break;
    case 5:
        if ((visibleSymbol[rowInd] & 0x01) > 0) COL6 = 1;
        else COL6 = 0;
        break;
    }
}
            ...
            if (cidx == 5) {       // column index
                ...
                if (ridx == 5) {    // row index
                    ...
                    if (RxReady == 1) {
                        ...
                        if (EYE_ID == 0 && (RxBuffer[0] & 0x40) > 0) {
                            for (int i = 0; i < 6; i++) {
                                visibleSymbol[i] = RxBuffer[i];
                            }
                        } else if (EYE_ID == 1 && (RxBuffer[0] & 0x80) > 0) {
                            for (int i = 0; i < 6; i++) {
                                visibleSymbol[i] = RxBuffer[i];
                            }
